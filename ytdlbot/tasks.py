...
def direct_normal_download(bot_msg, client, url):
    ...
    try:
        ...
        bot_msg.edit_text(f"✖️Download failed!❌\n\n```{e}```", disable_web_page_preview=True)
        return

    if not filename:
        filename = quote_plus(url)

    with tempfile.TemporaryDirectory(prefix="ytdlsl-") as f:
...
if __name__ == '__main__':
    ...
    scheduler = BackgroundScheduler(timezone="Asia/SriLanka")
